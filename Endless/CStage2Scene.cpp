...
void CStage2Scene::Render(ID3D11DeviceContext* _deviceContext)
{
	CScene::Render(_deviceContext);
}

void CStage2Scene::Enter()
{
	CScene::Enter();
...
void CStage2Scene::Exit()
{
	CScene::Exit();
}

void CStage2Scene::CreateStructure()
{
	// 바닥
...
void CStage2Scene::CreateMonster()
{
	...
	normMonster->SetPosition(XMFLOAT3(-5.5f, 2.0f, 5.5f));
	...
	// 몬스터 충돌체
	CCollider* monsterCollider1 = new CCollider;
	monsterCollider1->SetOwner(normMonster);
	monsterCollider1->SetScale(XMFLOAT3(0.5f, 0.5f, 0.5f));
	normMonster->SetComponent(monsterCollider1, COMPONENT_TYPE::COLLIDER);
	// 몬스터 상태
	CState** states4 = normMonster->GetStates();
	((CIdleState*)states4[(size_t)MONSTER_STATE::IDLE])->SetStayTime(2.0f);
	((CPatrolState*)states4[(size_t)MONSTER_STATE::PATROL])->SetPatrolPos(XMFLOAT3(-5.5f, 0.0f, 7.0f), XMFLOAT3(-5.5f, 0.0f, 4.0f));
	((CAttackState*)states4[(size_t)MONSTER_STATE::ATTACK])->SetAttackSpeed(1.0f);
	...
	CState** states5 = normMonster1->GetStates();
	...
	normMonster2->SetPosition(XMFLOAT3(5.5f, 2.0f, -5.5f));
	CState** states6 = normMonster2->GetStates();
	...
	CState** states7 = normMonster3->GetStates();
...
void CStage2Scene::CreateUI()
{
	...
	stage2Button->SetUIPos(XMFLOAT2(70.0f, 140.0f));
	stage2Button->SetUIWidth(50);
	stage2Button->SetUIHeight(50);
	stage2Button->SetParentUI(overPanel);
	...
	overPanel->PushChildUI(stage2Button);
